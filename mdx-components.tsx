...
export function useMDXComponents(components: MDXComponents): MDXComponents {
    ...
    return {
        ...
        li: ({ children }) => (
            <li className="grid grid-cols-[auto_1fr] before:content-['\007E'] before:leading-snug w-full before:text-yellow-400 before:pr-4">
        ...
        code: ({ children, className, ...rest }) => (
            <code
                className={
                    (className ?? "") +
                    singleWhiteSpace +
                    "font-mono *:leading-none [&:not(.hljs)]:text-yellow-400 line-number:content-[attr(data-line-number)] line-number:inline-block line-number:w-5 line-number:mr-4 line-number:text-right line-number:text-gray-500 line-number:leading-none"
                }
                {...rest}
            >
                {children}
